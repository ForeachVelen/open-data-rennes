...
class Slice {
    ...
    /**
     * Rewrites Blade @can statement into valid PHP
     *
     * @param string    $content
     * @return string|string[]|null
     */
	protected function _compile_can($content)
    {
        $pattern = '/(\s*)@can(\s*\(.*\))/';

        return preg_replace($pattern, '<?php if(Middleware::can($2)): ?>', $content);
    }

    /**
     * Rewrites Blade @endcan statement into valid PHP
     *
     * @param $content
     * @return mixed
     */
    protected function _compile_endcan($content)
    {
        return str_replace('@endcan', '<?php endif; ?>', $content);
    }
